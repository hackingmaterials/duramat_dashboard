...
def calc_all_window_metrics(df, window, meas_col, model_col, overwrite=True):
    ...
    df['GHI>0'] = (df[meas_col] > 5).astype(int)
...
def detect_clearsky(measured, clearsky, times, window_length,
    ...
    """
    ** MODIFIED **
    Components returns numerical values for each feature, NOT whether or not the window 'passes'.
...
def pvlib_features(measured, clearsky, times, window_length):
    """Modified function (from pvlib.detect_clearsky) to perform feature extraction.
    ...
    -------
    components : OrderedDict
        Dict of arrays of value for each parameter.
